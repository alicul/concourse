...
var _ = Describe("Pipeline user_data", func() {
	...
	Context("when setting a pipeline with user_data as a map", func() {
		...
		BeforeEach(func() {
			...
			_, _, _, err := team.CreateOrUpdatePipelineConfig(atc.PipelineRef{Name: pipelineName}, "0", config, false)
			Expect(err).NotTo(HaveOccurred())
		})

		AfterEach(func() {
			err := team.DeletePipeline(pipelineName)
			Expect(err).NotTo(HaveOccurred())
		})
	...
	Context("when updating a pipeline's user_data", func() {
		...
		It("replaces user_data with new value", func() {
			...
			_, _, _, err = team.CreateOrUpdatePipelineConfig(atc.PipelineRef{Name: pipelineName}, version, currentConfig, false)
			Expect(err).NotTo(HaveOccurred())
